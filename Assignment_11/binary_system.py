...
class BINARY(object):
    ...
    '''
    def __init__(self, _m1=4*pi, _m2=4*pi, _v10=[0,1./3.*sqrt(6.*pi)], _v20=[0,-2./3.*sqrt(6.*pi)], _dt=0.0001, _time=5):
        self.m1=_m1 
        self.m2=_m2
    ...
    def plot_trajectory(self,_ax):
        ...
        _ax.plot([self.x1[0]],[self.y1[0]],'ob',markersize=8)
        _ax.plot([self.x2[0]],[self.y2[0]],'or',markersize=8)
...
ax1.text(-0.5,0.1,'$M_1/M_2=1$',fontsize=18)
ax2.text(-2.2,0.1,'$M_1/M_2=1$',fontsize=18)

cmp=BINARY()
cmp.cal()
cmp.plot_trajectory(ax1)

cmp=BINARY(4*pi,4*pi,[1./3.*sqrt(6*pi)*sin(pi/6.),1./3.*sqrt(6*pi)*cos(pi/6.)],[-2./3.*sqrt(6*pi)*sin(pi/6.),-2./3.*sqrt(6*pi)*cos(pi/6.)])
cmp.cal()
